...
\textcolor{blue}{Confounder} (or ``confounding variable"): is a variable that either:
\medskip
\begin{itemize}
	\item \textcolor{blue}{causes} the exposure and  \textcolor{blue}{causes} the outcome
	\medskip
	\item \textcolor{blue}{is associated with} the exposure and  \textcolor{blue}{causes} the outcome
	\medskip
	
	\item or \textcolor{blue}{causes} the exposure and  \textcolor{blue}{is associated with} the outcome
	\medskip
...
\centering \includegraphics[scale=0.4]{figures/confounder1.png}

\end{frame}

\begin{frame}{Confounders}
	\centering \includegraphics[scale=0.6]{figures/confounder1_eg}
\end{frame}
...
\centering \includegraphics[scale=0.4]{figures/confounder2.png}
\end{frame}

\begin{frame}{Confounders}
	\centering \includegraphics[scale=0.6]{figures/confounder2_eg}
\end{frame}

% third diagram example
\begin{frame}{Confounders}
	Causal diagrams are useful tools we can use to determine whether or not variables are confounders.
...
\end{itemize}
	...
	\dots or confounders can look like this\dots
	
	\vspace{0.3cm}
	
	\centering \includegraphics[scale=0.4]{figures/confounder4.png}
\end{frame}

\begin{frame}{Confounders}
	\centering \includegraphics[scale=0.6]{figures/confounder4_eg}
\end{frame}
...
\begin{figure}
	\centering \includegraphics[scale=0.4]{figures/confounder3.png}
\end{figure}
...
\begin{frame}{Confounders}
	\centering \includegraphics[scale=0.6]{figures/confounder3_eg}
\end{frame}
...
\begin{figure}
	\centering \includegraphics[scale=0.4]{figures/lemons.png}
\end{figure}
...
\begin{frame}{Confounders: Example}
	\vspace{-0.6cm}
\begin{figure}
	\centering \includegraphics[scale=0.3]{figures/lemons.png}
\end{figure}
...
\textcolor{blue}{Answer}: From the graph, we can see that highway fatality rates have decreased \textit{over time}, and number of lemons imported has increased \textit{over time}.

\medskip 
 Time is a confounding variable in this case. Highway fatality rates have likely decreased over time because cars have gotten much safer with improved airbag quality and other innovations. 
...
\begin{itemize}
\item a randomized controlled trial is the only study design in which we can confidently make causal claims
\medskip

 \item \textit{However}, if we are able to adjust for \textbf{all possible confounders} in an observational study, we could also make causal statements 
 \medskip
 
 \item In practice, situations where we can confidently say we've adjusted for all possible confounders are \textbf{extremely rare}
 \medskip
 
 \item Nevertheless, it is good to adjust for any possible confounders we have, so long as we are still answering a scientific question of interest. 
\end{itemize}
...
\textcolor{violet}{Pollev:} Why don't we have any confounders in experimental studies?

\url{https://PollEv.com/multiple_choice_polls/y3iWfbLQnajGOK30qkRyV/respond}\pause

\medskip

No variables can cause or be associated with the exposure because it is assigned by the experimenters (randomized).
\end{frame}
...
\begin{figure}
	\centering \includegraphics[scale=0.2]{figures/age_firstep.png}
\end{figure}
...
\centering \includegraphics[scale=0.4]{figures/confound_code.png}
\end{frame}
...
\centering \includegraphics[scale=0.4]{figures/confound_code2.png}
\end{frame}
...
\begin{figure}
	\centering \includegraphics[scale=0.4]{figures/effectmod1.png}
\end{figure}
\end{frame}
...
\begin{frame}{Effect modifiers}
	...
	\textcolor{blue}{Effect modifiers}: a variable that modifies the association between the predictor of interest and the outcome. In other words, the association between the predictor of interest and the outcome \textit{depends on} the effect modifier. 
	
	\vspace{0.3cm}
	
	Note that while we call them \textcolor{blue}{"effect"} modifiers, what is being modified is \textcolor{blue}{an association}, whether causal or not
...
\centering \includegraphics[scale=0.4]{figures/p2.png}
\end{frame}
...
\begin{figure}
	\centering \includegraphics[scale=0.4]{figures/p3.png}
\end{figure}
...
\begin{figure}
	\centering \includegraphics[scale=0.4]{figures/p4.png}
\end{figure}
...
$$
E[Y \mid X, Z] = \beta_0 + \beta_1 X + \beta_2 Z + \textcolor{blue}{\beta_3 (X \times Z)}
$$
\end{frame}
...
Thinking back to our \texttt{births} dataset, suppose we are interested in whether or not the association between age and birthweight \textcolor{blue}{differs for individuals in First Steps vs. those not in First Steps}. Since we are interested in how an association between a predictor of interest and outcome varies based on another variable, we are interested in \textcolor{blue}{effect modification}. \pause
...
\centering \includegraphics[scale=0.4]{figures/interact_mod.png}
\end{frame}
...
\centering \includegraphics[scale=0.4]{figures/interact_mod2.png}
\end{frame}
...
\centering \includegraphics[scale=0.37]{figures/effectmod2.png}
\end{frame}
...
\textcolor{blue}{Precision variable}: a variable that causally affects the outcome, but is not associated with the predictor of interest \pause
...
\centering \includegraphics[scale=0.4]{figures/precvar1.png}
...
\begin{itemize}
	\item \textcolor{red}{L}inearity: There is a linear relationship between \color{blue}$X_{1,i}, \dots, X_{p,i}$ \color{black} and the true population conditional mean \color{blue}$Y_i \mid X_{1,i}, \dots, X_{p,i}$
	\begin{itemize}
		\item i.e. \color{blue} $E[Y_i \mid X_{1,i}, \dots, X_{p,i}] = \beta_0 + \beta_1 X_{1,i} + \dots + \beta_p X_{p,i}$  \color{black}
	\end{itemize} 
\smallskip
...
Note that \textbf{the only assumption that changed is linearity}, we now require a linear relationship between \textcolor{blue}{all covariates} in our model and the true population conditional mean. \textbf{This assumption can still be checked using the same diagnostic plots as in Chapter 1!}
