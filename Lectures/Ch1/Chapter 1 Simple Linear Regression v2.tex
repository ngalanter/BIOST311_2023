...
\centering \includegraphics[scale=0.45]{atrophy_age_code}
...
\begin{frame}{Atrophy and Age Example}
	\begin{figure}
		\centering
		\includegraphics[scale = 0.5]{age_atrophy_resids_v_fitted}
	\end{figure}
\medskip
...
%
%\begin{frame}{Log transformations: $\beta_0$ interpretation}
%So far we've seen linear transformations. What about log transformations? Going back to the first steps example, consider the model
%$$
%E[\text{bwt} \mid \text{wpre}] = \beta_0 + \beta_1 \log(\text{wpre})
%$$
%where wpre = birth parent's pre-pregnancy weight in pounds.
...
%
%\includegraphics[scale=0.01]{technical}
%\begin{align*}
