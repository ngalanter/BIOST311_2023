...
\begin{frame}{The WCGS Study}
	...
	\textcolor{blue}{\textbf{Exercise:}} What study design is this?  
	\\~\
...
\begin{frame}{Linear regression with binary outcomes: inference}
	...
	\begin{itemize}
		\item Identify the regression coefficient of interest, $\beta$.   % which one(s) answer(s) our scientific Q?
		\item Report an estimate of $\beta$, and interpret: \textit{We estimate that the difference in probabilities between two groups...}  
		\item Report a 95\% confidence interval for $\beta$, and interpret: \textit{Based on a 95\% confidence interval, this observed difference in probabilities would not be judged unusual if...}  
		\item Report the p-value from a hypothesis test of $H_0: \beta = 0$: \textit{These data provide evidence to suggest that this difference in probabilities is (is not) significantly different from zero ($p =$...).}  
...
\begin{frame}{Linear regression with binary outcomes: Prediction}
	...
	\begin{itemize} \itemsep +5pt
		\item Get estimates for each regression coefficient: $\hat\beta_0, \cdots, \hat\beta_p$  
		\item Plug in those estimates, along with the covariate values for the new individual: $\hat\beta_0 + \hat\beta_1 x_1 + \cdots \hat\beta_p x_p$  
		\begin{itemize} \itemsep +5pt
			\item This is our best estimate of $Y$ for a person with $X_1 = x_1, \cdots X_p = x_p$ ($\hat{Y}$)  
...
\begin{frame}
	...
	\vspace{-0.5cm}
	\begin{small} What if we wanted to quantify the association between CHD and Type A behavior via the \textit{odds ratio} rather than the \textit{risk difference}? \end{small}   \vspace{0.1cm}

	\begin{enumerate}
			\item \textbf{Scientific Question:} is Type A behavior \textcolor{orange}{associated} with the risk of CHD?  
			\item \textbf{Statistical Question:} is the \textcolor{orange}{ratio of odds} of CHD between people with and without Type A behavior different from 1?  
			\begin{itemize}
					\item \textbf{Parameter:} odds ratio  
				\end{itemize}
			\item Take a \textbf{sample} from the population: cohort study of men in California aged 39-59  
...
\begin{frame}{Logistic regression: formulating a statistical question}
	\begin{enumerate}
		\item \textbf{Scientific Question:}   Is \textit{arcus senilis} associated with risk of CHD?  
		\item \textbf{Statistical Question:}   
		\begin{itemize}
			\item How is our outcome quantified? \begin{tiny}The study is already completed -- we don't really have any control over this!\end{tiny} 
			\item[] \textcolor{blue}{Binary: cardiovascular event = 1, no event = 0} 
			\item How will we quantify association?  
			\item[] \textcolor{blue}{Ratio of odds of CHD comparing groups with and without \textit{arcus senilis}} 
...
\begin{frame}{Symmetry of the odds ratio: Example}
	...
	\end{center}
	\begin{align*}
		\text{OR} = \frac{\frac{P(\text{cancer} \mid \text{smoke})}{1 - P(\text{cancer} \mid \text{smoke})}}{\frac{P(\text{cancer} \mid \text{no smoke})}{1 - P(\text{cancer} \mid \text{no smoke})}}  = \frac{\frac{184/186}{2/186}}{\frac{16/214}{198/214}} = \frac{184\times 198}{16 \times 2} = 1138.5\\  
...
\begin{frame}{Logistic regression in case-control studies}
	...
	\end{align*} 
	\begin{itemize}
		\item $e^{\beta_0}$: odds of cancer among nonsmokers 
		\begin{itemize}
